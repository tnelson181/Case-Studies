...
WHERE Bookings.facid IN (0,1) AND surname <> 'Guest'
...
SELECT firstname, surname, name, Facilities.membercost, Facilities.guestcost
FROM Members
INNER JOIN Bookings
ON Members.memid = Bookings.memid
INNER JOIN Facilities
...
INNER JOIN (SELECT *
FROM Bookings
WHERE DATE(starttime) = '2012-09-14') AS FilteredBookings 
ON Members.memid = FilteredBookings.memid
INNER JOIN Facilities ON Facilities.facid = FilteredBookings.facid
...
sql_query = """
SELECT *,
       (membercost + guestcost) AS revenue
FROM Facilities
...
SELECT m1.firstname AS member_firstname, 
       ...
       m2.surname AS recommended_by_lastname
FROM Members AS m1
